...
if __name__ == "__main__":
    ...
    try:
        os.mkdir("./results/realChannel/Haswell/")
    except:
        pass
    ...
    samples_num_vec = [500000]  # bars section
    stitch_shift_size = 1
    ...
    i = 1
    fileV_vec = ["0", "1", "2", "3", "4"]
    for fileV in fileV_vec:
        path = "./samples/Haswell/probe230/good_decoded_samples_v{0}.txt".format(fileV)
        p_list = [path]

        try:
            os.mkdir("./results/realChannel/Haswell/fileV{0}/".format(fileV))
        except:
            pass
        ...
        for window_size in window_size_vec:
            ...
            for samples_num in samples_num_vec:
                # f_data = open(f_data_path, "a+")
                summryMy = open(
                    "./results/realChannel/Haswell/fileV{6}/summryMy_keyLength={0}_windowSize={1}_quantile={2}_withOpposite={3}_start_samp={4}_sample_end={5}.txt".format(
                        key_length, window_size, quantile, opposite, start_samp, start_samp + samples_num, fileV), "a+")
                ...
                retrievedKeyRegularFile = open(
                    "./results/realChannel/Haswell/fileV{5}/retrievedKeyRegularFile={0}_windowSize={1}_quantile={2}_start_samp={3}_sample_end={4}.txt".format(
                        key_length, window_size, quantile, start_samp, start_samp + samples_num, fileV), "a+")
                ...
                retrievedKeyOppositeFile = open(
                    "./results/realChannel/Haswell/fileV{5}/retrievedKeyOppositeFile={0}_windowSize={1}_quantile={2}_start_samp={3}_sample_end={4}.txt".format(
                        key_length, window_size, quantile, start_samp, start_samp + samples_num, fileV), "a+")
