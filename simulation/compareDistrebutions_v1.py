...
def Compute_precitle_of_good_samples(allGoodPossible_dict, percentileArray, expNum, max, min, goodDidntShow, rezolution):
    """
    ...
    for goodSample in allGoodPossible_dict.keys():
        if allGoodPossible_dict[goodSample]["count"] == 0:
            goodDidntShow[expNum] += 1
        else:
            for i in range(1, rezolution):
                if allGoodPossible_dict[goodSample]["count"] > (i-1)*onePrecentile and  allGoodPossible_dict[goodSample]["count"] <= i*onePrecentile:
                    percentileArray[i-1][expNum] += 1
            if allGoodPossible_dict[goodSample]["count"] > (rezolution-1)*onePrecentile:
...
def Compute_statistics_for_spesific_error_type(type, numberOfExpiriments, key, fragment_len, fragments_number, window_size, allGoodPossible_dict, rezolution ):
    ...
    for expNum in xrange(0,numberOfExpiriments):
        ...
        percentileArray = Compute_precitle_of_good_samples(allGoodPossible_dict=allGoodPossible_dict,
