...
if __name__ == "__main__":
    ...
    SIMULATION = True # False -  this mean it will take samples from file
    GABI_SLOW = False
    MY_FAST = True
                        ...
                        for sample_len_value in sample_len_list:
                            for num_constant_value in num_constant_list:
                                for quantile_value in quantile_list:
                                    ...
                                    if GABI_SLOW:
                                        summryGabi = open("./results/summryGabi_key={0}_allowCycle={1}_shiftPointersMethod={2}_windowSize={3}_simulation={4}.txt".format(n,ALLOW_CYCLES, SHIFT_POINTERS_METHOD, window_size,SIMULATION),"a+")
                                        if not COMPRAE_GUBI_AND_MY:
                                            shift_pointers2 = func.build_shift_pointers3(common_samples_df, stitch_shift_size)

                                        retrieved_key2 = func.stitch(common_samples_df, shift_pointers2)
                                        candidate_key2 = max(retrieved_key2, key=len)

                                        string = "\n\n\n\n~~~~~~~~~~~~~~~~~~~~~~\nGABI_SLOW = " + str(GABI_SLOW)
                                        print string
                                        summryGabi.write(string)

                                        string = "\noriginal key(len={0}):\n".format(n) + key
                                        print string
                                        summryGabi.write(string)

                                        string = "\ncandidate_key(len={0}):\n".format(len(candidate_key2)) + candidate_key2
                                        print string
                                        summryGabi.write(string)

                                        string = "\nkey.find(candidate_key) = " + str(key.find(candidate_key2))
                                        print string
                                        summryGabi.write(string)

                                        string = "\nlevenshtein_edit_dist(key, candidate_key) = \n" + str(func.levenshtein_edit_dist(key, candidate_key2))
                                        print string
                                        summryGabi.write(string)

                                        # for cand in retrieved_key2:
                                        #     if len(cand) > n / 3:
                                        #         f_candidates2.write(cand)
                                        #         f_candidates2.write("\n")
                                        # f_candidates2.close()
    ...
    if not SIMULATION:
        ...
        window_list = [19, 27, 20, 28, 21, 29, 22, 30, 25]
            ...
            for window_size_value in window_list:
                for stitch_shift_size_value in stitch_list:
                    for quantile_value in quantile_list:
                        ...
                        result_df = func.build_samples_from_file(pathlist=pathlist, window_size=window_size)
                        ...
                        if MY_FAST:
                            summryMy = open("./results/summryMy_key={0}_allowCycle={1}_shiftPointersMethod={2}_windowSize={3}_simulation={4}.txt".format(n,ALLOW_CYCLES, SHIFT_POINTERS_METHOD, window_size,SIMULATION),"a+")
                            if SHIFT_POINTERS_METHOD == "count":
                                shift_pointers = func.build_shift_pointers_order(common_samples_df, stitch_shift_size, ALLOW_CYCLES)
                            if SHIFT_POINTERS_METHOD == "position":
                        ...
                        if GABI_SLOW:
                            ...
                            retrieved_key2 = func.stitch(np.array(common_samples_df['sample']), shift_pointers2, ALLOW_CYCLES)
                            candidate_key2 = max(retrieved_key2, key=len)
